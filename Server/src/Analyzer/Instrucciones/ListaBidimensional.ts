...
export default class ListaBidimensional extends Instruccion {
    private id: string;
    ...
    interpretar(arbol: Arbol, tabla: TablaSimbolos) {
        ...
        for (let i = 0; i < this.valores.length; i++) {
            ...
            for (let j = 0; j < this.valores[i].length; j++) {
                const expr = this.valores[i][j];
                const resultado = expr.interpretar(arbol, tabla);
                if (resultado instanceof Errores) return resultado;

                if (expr.tipo_dato.getTipo() !== this.tipoElemento.getTipo()) {
                    return new Errores("Semántico", `Tipo incompatible en lista ${this.id}[${i}][${j}]: se esperaba ${this.tipoElemento.getTipo()}, pero se obtuvo ${expr.tipo_dato.getTipo()}`, this.linea, this.columna);
                }
